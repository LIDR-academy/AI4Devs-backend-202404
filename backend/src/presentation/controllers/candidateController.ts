...
export const updateInterviewStepController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (isNaN(candidateId) || isNaN(applicationId) || isNaN(newStepId)) {
      return res.status(400).json({ error: 'Invalid input' });
    }
    await updateInterviewStep(candidateId, applicationId, newStepId);
    res.status(204).send();
  } catch (error) {
    if (error instanceof Error) {
