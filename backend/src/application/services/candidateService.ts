...
export const updateInterviewStep = async (
  ...
  if (!application || application.candidateId !== candidateId) {
    throw new Error('Application not found or candidate mismatch');
  }
  if (newStepId <= application.currentInterviewStep) {
    throw new Error('New step must be greater than current step');
  }
